...
case class Player(id: Long,teamId: Long, name: String, surname: String, number: Long, isDeleted: Boolean = false) extends BaseEntity {
...
class PlayerTable(_tableTag: Tag) extends BaseTable[Player](_tableTag, None, "PLAYERS") {
  def * = (id,teamId, name, surname, number, isDeleted) <> (Player.tupled, Player.unapply)

  def ? = ( Rep.Some(id),Rep.Some(teamId),Rep.Some(name), Rep.Some(surname), Rep.Some(number), Rep.Some(isDeleted)).shaped.
  ...
  val teamId: Rep[Long] = column[Long]("teamId")
